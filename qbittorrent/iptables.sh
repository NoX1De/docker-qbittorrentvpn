...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	...
	# setup route for qBittorrent webui using set-mark to route traffic for port 8282 and 8999 to "${docker_interface}"
	echo "8282    webui" >> /etc/iproute2/rt_tables
...
iptables -A INPUT -i "${docker_interface}" -p tcp --dport 8282 -j ACCEPT
iptables -A INPUT -i "${docker_interface}" -p tcp --sport 8282 -j ACCEPT
...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	...
	iptables -t mangle -A OUTPUT -p tcp --dport 8282 -j MARK --set-mark 1
	iptables -t mangle -A OUTPUT -p tcp --sport 8282 -j MARK --set-mark 1
fi

# accept output from qBittorrent webui port - used for lan access
iptables -A OUTPUT -o "${docker_interface}" -p tcp --dport 8282 -j ACCEPT
iptables -A OUTPUT -o "${docker_interface}" -p tcp --sport 8282 -j ACCEPT
